...
## Prompt Engineer — The Context Alchemist
**Learning:** Payloads generated inside complex bookmarklets (`bookmarklets/job-clipper.js`) and auxiliary prompt files (`bookmarklets/prompts/standard-outputs.md`) lacked strict behavioral constraints (e.g. `<thinking>` blocks, explicit negative constraints, and domain-grounded expert personas). Without these constraints, models generate multi-paragraph prose and hallucinate unverified information under inference pressure.
**Action:** Injected the `// ⚙️ CORTEX x AUTHOR` header, strict domain-grounded expert personas, explicit negative constraints, and structured schema requirements into `bookmarklets/prompts/standard-outputs.md`, the Questionnaire Extractor payload, and the Data Extractor payload in `bookmarklets/job-clipper.js`.
