...
async function runTest() {
    ...
    const originalWait =
        /const wait\s*=\s*ms\s*=>\s*new\s*Promise\s*\(\s*r\s*=>\s*setTimeout\s*\(\s*r\s*,\s*ms\s*\)\s*\);/;
    code = code.replace(originalWait, 'const wait = ms => new Promise(r => setTimeout(r, 10));');
    ...
    code = code.replace('&& el.offsetParent !== null', '');
