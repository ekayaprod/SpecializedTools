(function () {
    ...
    function find(query) {
        ...
        /* ZIP SEARCH */
        if (/^\d{5}$/.test(cleanedQuery)) {
            const zipCode = parseInt(cleanedQuery, 10);

            /* CITY_OVERRIDES Specifics */
            for (const city in CITY_OVERRIDES) {
                const counties = CITY_OVERRIDES[city];
                for (const county in counties) {
                    if (counties[county].includes(zipCode)) {
            ...
            if (results.length > 0) {
                return cleanedQuery + ': ' + results.join(', ');
            }

            /* COUNTY_DATA Ranges */
            for (let i = 0; i < COUNTY_DATA.length; i++) {
                const rangeStart = /** @type {number} */ (COUNTY_DATA[i][1]);
                const rangeEnd = /** @type {number} */ (COUNTY_DATA[i][2]);
                if (zipCode >= rangeStart && zipCode <= rangeEnd) {
                    if (!results.includes(COUNTY_DATA[i][0])) results.push(COUNTY_DATA[i][0]);
                }
            }

            return results.length ? cleanedQuery + ': ' + results.join(', ') : null;
        }

        /* CITY SEARCH */
        const lowerQuery = cleanedQuery.toLowerCase();

        /* CITY_OVERRIDES Search (Overrides) */
        if (Object.prototype.hasOwnProperty.call(CITY_OVERRIDES, lowerQuery)) {
            const counties = CITY_OVERRIDES[lowerQuery];
            for (const county in counties) {
                if (!results.includes(county)) results.push(county);
            }
        }
        ...
        /* COUNTY_DATA Search */
        for (let i = 0; i < COUNTY_DATA.length; i++) {
            const cities = /** @type {string[]} */ (COUNTY_DATA[i][3]);
            for (let j = 0; j < cities.length; j++) {
                if (cities[j] === lowerQuery) {
                    if (!results.includes(COUNTY_DATA[i][0])) results.push(COUNTY_DATA[i][0]);
                    break;
                }
            }
    ...
    if (s) {
        ...
        const closeButton = createCloseBtn();
